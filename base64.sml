...
structure Base64 : BASE64 =
  ...
  (* encoding table *)
    val encTbl = "\
	  ...
	  \"
    val padChar = #"="
    fun incByte b = String.sub(encTbl, Word8.toIntX b) 
  ...
  (* encode a byte into two base-64 characters plus two padding characters *)
    ...
    local
      fun encode64 (vec, start, len) = let
	    ...
	    val outBuf = CA.array (outLen, chr 0)
	    val nTriples = Int.quot(len, 3)
	    val extra = Int.rem(len, 3)
	    fun insBuf (i, (c1, c2, c3, c4)) = let
		  val idx = 4*i
		  in
		    CA.update(outBuf, idx,   c1);
		    CA.update(outBuf, idx+1, c2);
		    CA.update(outBuf, idx+2, c3);
		    CA.update(outBuf, idx+3, c4)
		  end
	    fun loop (i, idx) = if (i < nTriples)
		  then (
		    insBuf(i, encode3(W8V.sub(vec, idx), W8V.sub(vec, idx+1), W8V.sub(vec, idx+2)));
		    loop (i+1, idx+3))
		  else (case extra
		     of 1 => insBuf(i, encode1(W8V.sub(vec, idx)))
		      | 2 => insBuf(i, encode2(W8V.sub(vec, idx), W8V.sub(vec, idx+1)))
		      | _ => ()
		    (* end case *))
	    in
	      loop (0, start);
